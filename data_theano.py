...
def load_my_data(train_x_path, train_y_path, test_x_path, test_y_path, valid_x_path, valid_y_path, shared = False):
    ...
    if shared:
        train_set_x, train_set_y = shared_dataset((loadtxt(train_x_path)[:-1], quantize(loadtxt(train_y_path)[:-1].T)))
        test_set_x, test_set_y = shared_dataset((loadtxt(test_x_path), quantize(loadtxt(test_y_path).T)))
        valid_set_x, valid_set_y = shared_dataset((loadtxt(valid_x_path)[:-9], quantize(loadtxt(valid_y_path)[:-9].T)))
