...
if [[ ("$ver" == "vec") || ("$ver" == "all") ]]; then
    ...
    export mod
    export AV
    parallel --noswap --gnu -j+0 --eta --header : 'bash $AV/arg22vec.sh {filea} "$mod" w' ::: filea `ls "$stsdir"/split_sts/a_*.txt.out`
    parallel --noswap --gnu -j+0 --eta --header : 'bash $AV/arg22vec.sh {fileb} "$mod" w' ::: fileb `ls "$stsdir"/split_sts/b_*.txt.out`
fi

if [ -z "$v" ]; then
    export AV
    parallel -t -k --noswap --gnu -j+0 --eta --header : 'python $AV/trip_comp.py -A {filea}' ::: filea `ls "$stsdir"/split_sts/a_*.txt.out`
else
    export AV
