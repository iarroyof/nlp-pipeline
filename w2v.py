...
def clean_Ustring_fromU(string):
    ...
    try: # Trying different cases for bad input documents.
        ...
        try:
            ...
            sys.stderr.write('The wrong string at the second attempt: before %s words' % range_error)
            try:
                range_error = 99
                normalized_string = normalize('NFKC', gClean[0:range_error].lower())
            except TypeError:
                sys.stderr.write('The wrong string at the third attempt: before %s words' % range_error)
                try:
                    range_error = 49
                    normalized_string = normalize('NFKC', gClean[0:range_error].lower())
                except TypeError:    
                    sys.stderr.write('It was not possible forming output file after three attempts. Fatally bad file')
...
if __name__ == "__main__":
    ...
    parser.add_argument('-i', type=str, dest = 'indirname', help='Specifies the directory containing files to be processed.')
