...
def test_mlp(learning_rate=0.01, L1_reg=0.00, L2_reg=0.0001, n_epochs=1000,
   ...
   """
    ...
    tr_px = path + "pairs-SI/vectors_H50/pairs_eng-SI-test-2e6-nonempty_d2v_H50_sub_m5w8.mtx"
    tr_py = path + "pairs-SI/STS.gs.all-eng-SI-test-nonempty.txt"
    ts_px = path + "pairs-NO/vectors_H50/pairs_eng-NO-test-2e6-nonempty-halpf0_d2v_H50_sub_m5w8.mtx"
    ts_py = path + "pairs-NO/STS.gs.all-eng-NO-test-nonempty-half0.txt"
    vl_px = path + "pairs-NO/vectors_H50/pairs_eng-NO-test-2e6-nonempty-halpf1_d2v_H50_sub_m5w8.mtx"
    ...
    #n_train_batches = train_samples // batch_size
    #valid_samples = valid_set_x.shape[0]
    #n_valid_batches = valid_samples // batch_size
    #test_samples = test_set_x.shape[0]
    #n_test_batches = test_samples // batch_size
    #in_dimensions = train_set_x.shape[1]
    ...
    #assert train_set_x.shape[1] == valid_set_x.shape[1] == test_set_x.shape[1] # verify dimensions
    ...
    #assert train_samples == train_set_y.shape[0]
    #assert test_samples == test_set_y.shape[0] 
    #assert valid_samples == valid_set_y.shape[0] # verify samples and examples

    #k_classes = max(train_set_y) + 1 # for 0-based class index
    k_classes = T.max(train_set_y, axis=0).eval() + 1
    print ("classes: %d" % k_classes)
