...
def test_mlp(learning_rate=0.01, L1_reg=0.00, L2_reg=0.0001, n_epochs=1000,
   ...
   """
    ...
    datasets = load_my_data(tr_px,tr_py,ts_px,ts_py,vl_px,vl_py)
    #datasets = load_data(dataset)

    train_set_x, train_set_y = datasets[0]
    valid_set_x, valid_set_y = datasets[1]
    test_set_x, test_set_y = datasets[2]
    #print '>>>>>>> Shapes: tr_x: %s, tr_y: %s,ts_x: %s, ts_y: %s, tvl_x: %s, vl_y: %s' % 
    #print (T.cast(train_set_x, "float32").eval().shape,
    #                T.cast(train_set_y, "float32").eval().shape, 
    #                T.cast(test_set_x, "float32").eval().shape,
    #                T.cast(test_set_y, "float32").eval().shape,
    #                T.cast(valid_set_x, "float32").eval().shape,
    #                T.cast(valid_set_y, "float32").eval().shape
    #                  ) #T.cast(x, 'int32')
